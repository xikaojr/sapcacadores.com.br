 ...
 * @subpackage Cielo
 * @author     Eric Silva
 * @author     Jhonny Makis [Alteracoes para o sistema novo]
 *
 */
class App_CartaoCredito_Cielo extends Itarget_CartaoCredito {
    ...
    /**
     * @var string $formaPagamento Debito, credito a vista ou parcelado
     */
    ...
    /**
     *
     * @var int $autorizar Autorizar transacao
     */
    private $autorizar = 2;
    ...
    /**
     * Hash do numero do cartao do portador.
     * @var string
     */
    private $pan;
   ...
   // const URL_PRODUCAO = 'https://ecommerce.cbmp.com.br/servicos/ecommwsec.do';
    const URL_PRODUCAO = 'https://cielo.itarget.com.br/cielo.php';
    ...
    /**
     *
     * @param string $mensagemCancelamento
     * @return Cielo
     */
    public function setMensagemCancelamento($mensagemCancelamento) {
     ...
     * @return string
     */
    public function getMensagemCancelamento() {
    ...
    /**
     *
     * @param string $pan Hash do numero do cartao do portador
     * @return Cielo
     */
    ...
    public function getUrlAutenticacao() {
        if(is_object($this->urlAutenticacao)) {
            return end($this->urlAutenticacao);
        }
    ...
    /**
     *
     * @param int $numeroParcelas Numero de parcelas
     * @return Cielo
     */
     ...
     * Para Elo, o valor sera sempre 3.
     *
     * @param int $autorizar
     * @return Cielo
     */
    ...
    /**
     *
     * Codigo numerico da moeda na ISO 4217.
     * Para o Real, o codigo e 986.
     *
     * @param int $autorizar
     * @return Cielo
     */
    ...
    /**
     *
     * URL da pagina de retorno e para essa tela que o fluxo sera retornado ao
     * fim da autenticacao e/ou autorizacao.
     *
    ...
    /**
     *
     * Debito: const PAGAMENTO_DEBITO
     * Credito a vista: 1
     * Parcelado: X, onde X e o numero de parcelas
     *
    ...
    public function setFormaPagamentoRetorno($formaPagamento) {
    ...
    public function setAfiliacao($afiliacao) {
        ...
        if (strlen($afiliacao) != 10)
            throw new Exception('Numero de Afilicacao invalido');
    ...
    public function realizarTransacao() {
        ...
        if (!$this->getValorTransacao())
            throw new Exception('O valor da transacao nao foi definido');

        if (!$this->getWebservice())
            throw new Exception('Endereco do webservice nao esta definido');
        ...
        if ($this->getNumeroParcelas() > 1 && null == $this->getTipoParcelamento())
            throw new Exception('Tipo de parcelamento nao definido');

        if ($this->getNumeroParcelas() > 1)
            $this->setTipoParcelamento(2);
        else 
            $this->setTipoParcelamento(1);
        ...
        if (strlen($this->getUrlRetorno()) < 1)
            throw new Exception('URL de retorno nao definida');
        ...
        if (null == $resposta || empty($resposta)) {
            throw new Exception('HTTP READ TIMEOUT - o Limite de Tempo da transacao foi estourado');
        }
    ...
    public function consultarTransacao($tid) {
        ...
        $this->setFormaPagamentoRetorno($resposta->$formaPagamento->produto);
        ...
        $this->setMensagemCancelamento($resposta->cancelamento->mensagem);
    ...
    private function xmlFormaPagamento() {
        return "
            <forma-pagamento>
                ...
                <produto>{$this->getTipoParcelamento()}</produto>
