            ...
            foreach ($this->campos_retorno['detalhe'][$line] as $k => $v) {
                ...
                if (count($verify) > 0) {
                    if (array_key_exists($k, $verify) and $verify[$k] != '' and $value != $verify[$k]) {
                        $this->error[] = "Erro na verificaçao: Linha {$line} - Caractere{$v['de']} com o tamanho {$v['tamanho']}<br />{$k}: No arquivo:{$value}; No Sistema:{$verify[$k]}";
                    }
                }
                ...
                // define o valor
                if (!isset($row['valor_acrescimo']))
                    $row['valor_acrescimo'] = 0;
