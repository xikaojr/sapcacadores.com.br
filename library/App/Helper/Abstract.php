...
abstract class App_Helper_Abstract extends Zend_View_Helper_Abstract {
    ...
    public static function mostraMensagemBanco($exception) {
        ...
        if (strpos($exception, 'SQLSTATE') === false) {
            ...
            $exception = str_replace('ERROR', 'ERRO', $exception);
            preg_match('/ERRO: (.*)/i', $exception, $mensagemRetorno);
            preg_match('/SQL statement(.*)/', $exception, $sqlRetorno);
            $rs = trim($mensagemRetorno[1]);

            if (APP_ENV != 'prodution' && false) {
    ...
    public static function getNomeCampoConfiguracoes($codigo, $valueCampo) {
        ...
        $configuracoesTable = new ConfiguracaoTable();
        $configuracao = $configuracoesTable->findAllByCodigo($codigo);
